<template>
  <div v-if="options" class="list-edit-datatype-chip-outline">
    <div
      v-for="option in options"
      :key="option.value"
      :class="`list-edit-datatype-chip-outline__item indicator-chip-outline-color-${option.color}`"
    >
      <div class="list-edit-datatype-chip-outline__item__value">{{ option.value }}</div>
    </div>
    <div
      v-for="option in options"
      :key="option.value"
...
export default defineComponent({
  name: 'ListEditDatatypeChipOutline',
  props: {
    rowData: Object,
    cellHeader: Object
  },
...
.list-edit-datatype-chip-outline__item {
  display: flex;
  gap: 4px;
  padding: 4px 8px;
  border-radius: 4px;
